...
#Right machine
ugv_private, ugv_public = dh.generate_keys(p, g)
        ...
        try:
            ...
            try:
                ...
                if decode_message == 'NICK':
                    client.send(f"{nickname}:{ugv_public}".encode('utf-8'))
                elif decode_message.startswith('Public key'):
                    mtosplit = decode_message.split(':')
                    uav_pubkey_str = mtosplit[1]
                    uav_pubkey = int(uav_pubkey_str)
                    shared_key = dh.compute_shared(uav_pubkey, ugv_private, p)
