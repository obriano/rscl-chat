...
int encrypt(unsigned char* message, unsigned char* key,
            unsigned char* nonce, unsigned char* cipher)
{   

    unsigned long long clen;

    int result = crypto_aead_encrypt(cipher, &clen,
            ...
            unsigned char* decrypted)
{
    ...
    int result = crypto_aead_decrypt(decrypted, &dlen,
                        NULL, cipher,
                        strlen(cipher), NULL,
                        0, nonce,
                        key);
...
{
    printf("Basic encryption and decryption test\n");
    printf("Courtesy of RSCL at CPP\n");
    printf("Written by Omar Briano\n");
    printf("*****************************************************\n");

    int MAX_CIPHER_SIZE = 128+16;
    int MAX_MESSAGE_SIZE = 128;

    unsigned char message[] = "Hello from RSCL";
    unsigned char nonce[16] = "ABCDEFABCDEFABCD";
    unsigned char key[32] = "ABCDEFABCDEFABCDABCDEFABCDEFABCD";
    
    unsigned char cipher[MAX_CIPHER_SIZE];

    encrypt(message, key, nonce, cipher);
    ...
    unsigned char decrypted_message[MAX_MESSAGE_SIZE];
    
    decrypt(cipher, key, nonce, decrypted_message);
