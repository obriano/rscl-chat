...
#Right machine
uav_private, uav_public = dh.generate_keys(p, g)
        ...
        try:
            ...
            try:
                ...
                if decode_message == 'NICK':
                    client.send(f"{nickname}:{uav_public}".encode('utf-8'))
                elif decode_message.startswith('Public key'):
                    mtosplit = decode_message.split(':')
                    ugv_pubkey_str = mtosplit[1]
                    ugv_pubkey = int(ugv_pubkey_str)
                    shared_key = dh.compute_shared(ugv_pubkey, uav_private, p)
